...
if(isset($_POST['aux_send_form'])){
    ...
    require_once (FUNCIONES_FRAMEWORK_PROY. "token_afip.php");
    $res=CredentialsVerification();
    if ( $res !== "OK" ){
		die($res);
        $jquery_panel = "bootbox.alert('Se ha producido un error al autenticar su conexión con AFIP. Por favor, ingrese nuevamente al sistema con clave AFIP o ASIP.');";
        oci_close($coon);
    }
     else{
		$res = redirectUserAFIP();
        if($res == 'ERROR'){
