$(document).ready(function () {
	...
	$("#d_denominacion","#frm_busqueda").autocomplete({
		...
		select:function(event,ui){
			...
			$("#f_ultimo_certif").val(ui.item.f_ultimo);
			return false;
		}
	});
	...
	$("#lupa_c_tributo").lupa_generica({
		...
		onClose:function(){
			$("#d_solicitante, #n_plano_mensura, #d_observaciones").val(null);
			if ($("#c_tributo").val() == 60){
				$("#d_solicitante, #n_plano_mensura, #d_observaciones").attr('disabled',false);
			}else{
				if ($("#c_tributo").val() == 90){
					$("#n_plano_mensura").attr('disabled',true);
					$("#d_observaciones").attr('disabled',false);
				}else{
					$("#d_solicitante, #n_plano_mensura, #d_observaciones").attr('disabled',true);
				}
			}
		}
	});
