...
if ($p_oper === 'cuit'){
    $db_query = new DB_Query(
		"SELECT d_denominacion,id_contribuyente,c_tipo_documento, n_documento,
			(select d_dato from tablas_generales where n_tabla=1 and c_dato = c.c_tipo_documento) d_tipo_documento,
			(SELECT max(f_otorgamiento)
			from libre_deuda
			where id_contribuyente = c.id_contribuyente
			and N_CERTIF_FISCAL is not null) as fecha_ultimo
...
if($p_oper === 'denominacion'){
    ...
    for ($i=0; $i < count($row_query); $i++)
    {
        $options['data_contrib'][] = array(
            ...
            'f_ultimo' => $row_query[$i]['FECHA_ULTIMO']
        );
    }
