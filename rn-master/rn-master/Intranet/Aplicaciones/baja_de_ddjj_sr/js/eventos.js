...
function inicializa_eventos(){
    ...
    $('#btn_limpiar').click(function(){
        ...
        $('#mascara_lupa_d_concepto').hide();
        $('#lupa_d_concepto').show().css('display', 'table-cell');
        $('#lupa_objeto_hecho').hide();
        $('#mascara_lupa_objeto_hecho').show().css('display', 'table-cell');
        ...
        setea_parametros('#main_grid',{':p_id_contribuyente':null,
            ':p_N_POSICION_FISCAL':null,
            ':p_n_cuota_anticipo':null,
            ':p_c_tributo':null,
    ...
    $('#btn_buscar').click(function(){
        ...
        if (n_posicion_fiscal == '' ) {
            ...
            mostrar_cuadro('E', 'Error', 'Debe cargar un Nro. Cuota.');
            $('#main').procOverlay({visible:false});
        }else if(n_cuit == ''){
            ...
            $.ajax({
                ...
                success: function( data ) {
                    ...
                    if(data) {
                        ...
                        if ($('#c_tipo_documento').val() != '') filtros_arr_main.push('Tipo Documento: ' + $('#c_tipo_documento').val() + ' - '+ $('#d_tipo_documento').val());
                        if ($('#nro_documento').val() != '') filtros_arr_main.push('Nro. Documento: ' + $('#nro_documento').val());
                        if ($('#c_tributo').val() != '') filtros_arr_main.push('Tributo: ' + $('#c_tributo').val() + ' - ' + $('#d_tributo').val());
                        if ($('#c_concepto').val() != '') filtros_arr_main.push('Concepto: ' + $('#c_concepto').val() + ' - ' + $('#d_concepto').val());
                        if ($('#n_posicion_fiscal').val() != '') filtros_arr_main.push('Posición Fiscal: ' + $('#n_posicion_fiscal').val());
                        if ($('#n_cuota').val() != '') filtros_arr_main.push('Nro. Cuota: ' + $('#n_cuota').val());
                        ...
                        $('#main').procOverlay({visible:false});
                        mostrar_cuadro('I', 'Información', 'No hay registros para los datos ingresados.');
                    }
                }
            });
...
function inicializa_lupas(){
    ...
    $("#lupa_d_denominacion").lupa_generica({
        ...
        titulos:['ID_contribuyente','CUIT', 'Denominación', 'Código Tipo Doc.','Tipo Documento', 'Número Documento', 'F. Alta'],
        grid:[{index:'id_contribuyente',width:100, hidden: true},
            {index:'n_cuit',width:140},
            {index:'d_denominacion',width:400},
            {index:'c_tipo_documento',width:185, hidden: true},
            {index:'d_tipo_documento',width:175},
...
function bloquea_campos() {
    ...
    $('#lupa_d_concepto').hide();
    $('#mascara_lupa_d_concepto').show().css('display', 'table-cell');
    $('#lupa_objeto_hecho').hide();
    $('#mascara_lupa_objeto_hecho').show().css('display', 'table-cell');
    $('#lupa_d_denominacion').hide();
    $('#mascara_lupa_d_denominacion').show().css('display', 'table-cell');
