...
if ($oper === 'getContribuyente'){
    ...
    contribuyentes_tributos ct
    where c.id_contribuyente = ct.id_contribuyente 
    and ct.c_tipo_imponible in(select c_dato
                           from tablas_generales tg,
                                parametros        p
                           where tg.n_tabla = p.n_tabla
                           and c_constante  = 'TIMPO'
                           and d_dato1      = 'O')
    and c.n_cuit = :filtro
    and rownum = 1");
...
if ($oper === 'getAutocomplete'){
    $db_query = new DB_Query("SELECT DISTINCT c.id_contribuyente,
                                    fun_formato_cuit (c.n_cuit) as cuit, 
                                    c.d_denominacion as denominacion,
                                    c.c_tipo_documento,
                                    (SELECT 
                            ...
                            and ct.c_tipo_imponible in(select c_dato
                                                ...
                                                and c_constante  = 'TIMPO'
                                                and d_dato1      = 'O')
                            and substr (fun_transformar_cadena (c.d_denominacion), 1, 200) like fun_transformar_cadena ('%' || :filtro || '%')
                            and rownum <= 5");
...
if($oper == 5){
    ...
    $db_query = new DB_Query("select 
                            ...
                            and ct.c_tipo_imponible in(select c_dato
                                                       ...
                                                       and c_constante  = 'timpo'
                                                       and d_dato1      = 'o')
                            and c.c_tipo_documento = :c_tipo_documento
