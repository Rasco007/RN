	...
	if (($cuit!='') or (($t_doc!='') and ($n_doc!=''))){
		...
		if ($row_query_existe[0]['EXISTE'] == -1){
			...
			$db_query = new DB_Query(
				"SELECT      d_denominacion, 
				fun_formato_cuit(n_cuit) as n_cuit,
				case 
					when n_documento is null 
						then null
					else c_tipo_documento end c_tipo_documento,
				case 
					when n_documento is null 
						then null
					else (select d_dato from tablas_generales where n_tabla=1 and c_dato = c_tipo_documento) end d_c_tipo_documento,                  
