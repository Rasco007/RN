<?php
	class TDIExcelXLSX{
		...
		private $_style =
			...
			<styleSheet xml:space="preserve" xmlns="http://schemas.openxmlformats.org/spreadsheetml/2006/main">
				...
				<fills count="3">
					...
					<fill>
						<patternFill patternType="gray125">
							<fgColor rgb="FF004C99"/>
							<bgColor rgb="FF004C99"/>
						</patternFill>
					</fill>
				</fills>
				<borders count="2">
		...
		public function downloadExcel($sql,$parametres,$arrTitles,$filterArr,$consulta){
			...
			$this->addFromQuery($arrTitles,$sql,$parametres,$filterArr,$consulta);
		...
		private function addFromQuery($arrTitles,$sql,$parametres,$filterArr,$consulta){
			...
			$lastRow = $total + 1 + count($filterArr) + 2 + 2;
			...
			$firstRow = '<row r="'.(count($filterArr)+5).'" spans="1:'.count($arrTitles).'">';
			$countCol = 1;
			...
			foreach($arrTitles as $reg){
				foreach($reg as $title => $typeTitle){
					...
					if ($countCol == 1){
						$firstRow = $firstRow.'<c r="'.$this->_letters[($countCol - 1)].(count($filterArr)+5).'" s="1" t="s"><v>'.($countCol -1).'</v></c>';
					} elseif ($countCol == $maxCell) {
						$firstRow = $firstRow.'<c r="'.$this->_letters[($countCol - 1)].(count($filterArr)+5).'" s="3" t="s"><v>'.($countCol -1).'</v></c>';
					} else {
						$firstRow = $firstRow.'<c r="'.$this->_letters[($countCol - 1)].(count($filterArr)+5).'" s="2" t="s"><v>'.($countCol -1).'</v></c>';
					}
			...
			$rowIndex = 1;
			if (!is_null($consulta)){
				$filterRows = $filterRows .'<row r="'.$rowIndex.'" spans="1:2">';
				$filterRows = $filterRows .'<c r="A'.$rowIndex.'" s="1" t="s"><v>'.($countCol -1).'</v></c>';
				$this->_sharedStrings = $this->_sharedStrings ."<si><t>Consulta: </t></si>";
				$countCol++;
				$filterRows = $filterRows .'<c r="B'.$rowIndex.'" t="s"><v>'.($countCol -1).'</v></c>';
				$this->_sharedStrings = $this->_sharedStrings ."<si><t>".str_replace($replace_arr,'',str_replace('>','&gt;',str_replace('<','&lt;',str_replace('&','&amp;',$consulta))))."</t></si>";
				$countCol++;
				$filterRows = $filterRows."</row>"."\n";
				$this->_sharedStrings = $this->_sharedStrings ."\n";
				$rowIndex++;
			}
			$rowIndex++;
			if (count($filterArr)){
				$filterRows = $filterRows .'<row r="'.$rowIndex.'" spans="1:2">';
				$filterRows = $filterRows .'<c r="A'.$rowIndex.'" s="1" t="s"><v>'.($countCol -1).'</v></c>';
				$this->_sharedStrings = $this->_sharedStrings ."<si><t>Parámetro</t></si>";
				$countCol++;
				$filterRows = $filterRows .'<c r="B'.$rowIndex.'" s="3" t="s"><v>'.($countCol -1).'</v></c>';
				$this->_sharedStrings = $this->_sharedStrings ."<si><t>Valor</t></si>";
				$countCol++;
				$filterRows = $filterRows."</row>"."\n";
				$this->_sharedStrings = $this->_sharedStrings ."\n";
				$rowIndex++;
			...
			foreach($filterArr as $filter => $value){
				$filterRows = $filterRows .'<row r="'.$rowIndex.'" spans="1:2">';
				$filterRows = $filterRows .'<c r="A'.$rowIndex.'" t="s"><v>'.($countCol -1).'</v></c>';
				$this->_sharedStrings = $this->_sharedStrings ."<si><t>".str_replace($replace_arr,'',str_replace('>','&gt;',str_replace('<','&lt;',str_replace('&','&amp;',$filter))))."</t></si>";
				$countCol++;
				if ($value != null){
					$filterRows = $filterRows .'<c r="B'.$rowIndex.'" t="s"><v>'.($countCol -1).'</v></c>';
					$this->_sharedStrings = $this->_sharedStrings ."<si><t>".str_replace($replace_arr,'',str_replace('>','&gt;',str_replace('<','&lt;',str_replace('&','&amp;',$value))))."</t></si>";
					$countCol++;
				}
				else {
					$filterRows = $filterRows .'<c r="B'.$rowIndex.'"></c>';
				}
				$filterRows = $filterRows."</row>"."\n";
				$this->_sharedStrings = $this->_sharedStrings ."\n";
				$rowIndex++;
			...
			$this->_worksheet = $this->_worksheet . '</cols><sheetData>'.$filterRows.$firstRow.'</row>';
							...
							if ($value != null){
								if ($type_col == 'number') {
									if (strpos($value,',') === false){
										$rowWorksheet = $rowWorksheet .'<c r="'.$this->_letters[$key].$rowIndex.'"><v>'.($value).'</v></c>';
									} else {
