...
if ($p_oper === 'cuit'){
	if($abm == 'S'){
	    $db_query = new DB_Query(
			"SELECT id_contribuyente, null id_contribuyente_tmp, fun_formato_cuit(n_cuit) as cuit, d_denominacion as denominacion, c.c_tipo_documento,
					(select d_dato from tablas_generales where n_tabla=1 and c_dato = c.c_tipo_documento) d_c_tipo_documento, fun_formato_numerico(c.n_documento,0) n_documento
			from contribuyentes c where n_cuit = :filtro
			union all
			SELECT null id_contribuyente, id_contribuyente_tmp, fun_formato_cuit(n_cuit) as cuit, d_denominacion as denominacion, c.c_tipo_documento,
					(select d_dato from tablas_generales where n_tabla=1 and c_dato = c.c_tipo_documento) d_c_tipo_documento, fun_formato_numerico(c.n_documento,0) n_documento
			from tmp_contribuyentes c where n_cuit = :filtro"
		);
	}else{
		$db_query = new DB_Query(
			"SELECT id_contribuyente, null id_contribuyente_tmp, fun_formato_cuit(n_cuit) as cuit, d_denominacion as denominacion, c.c_tipo_documento,
...
if($p_oper === 'denominacion'){
	if($abm == 'S'){
		...
		FROM (
			SELECT null id_contribuyente, id_contribuyente_tmp, ' (TMP)' tmp, fun_formato_cuit(n_cuit) as cuit, d_denominacion as denominacion,
			...
			from tmp_contribuyentes c where substr (fun_transformar_cadena (d_denominacion), 1, 200) like fun_transformar_cadena ('%' || :filtro || '%')
			UNION ALL
