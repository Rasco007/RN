function validar_periodo(periodo_input, desde){
    ...
    if (periodo < 201206 && desde){
        mostrar_error('Las bonificaciones se pueden aplicar desde el 2012/06');
        periodo_input.val("");
        return;
    }

    if($('#periodo_desde').val() && $('#periodo_hasta').val()){
        if($('#periodo_hasta').val().replace("/", "") < $('#periodo_desde').val().replace("/", "")){
            mostrar_error('Periodo Hasta no puede ser anterior al Periodo Desde');
            $('#periodo_hasta').val("");
            $('#periodo_desde').val("");
            return;
        }
...
function aplicar(){
    $.ajax({
        ...
        dataType:'json',
        success: function( data ) {
            if(data.resultado == 'El proceso finalizó'){
