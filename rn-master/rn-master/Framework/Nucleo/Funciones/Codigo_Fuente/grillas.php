...
<script type="text/javascript">
	...
	var datos_detail_grid = new GridParam({id_menu:<?=$_POST['p_n_id_menu']?>,
		n_grid:1,
		m_autoquery:'N'});

	var copiar_campos_desde_tabla_grid = new GridParam({id_menu:<?=$_POST['p_n_id_menu']?>,
		n_grid:2,
		m_autoquery:'N'});

	var movio_columnas = false;// cuando mueve algun columna para reordenar pone en true

	$(document).ready(function() {

		// definición de variable de impresión
		var content="<div id='div_busqueda'></div>"+
			"<br />"+
			...
			"<div style='height:20px; width:100%'><div id='mensaje' style='text-align: left; display:none; height:15px; padding-top:5px'>Mueva los registros y luego aplique los cambios para que surjan efecto.</div></div>"+
			'<div id="dialog_copiar_grilla" title="Copiar Grilla" style="display:none;">'+
			'<form id="frm_copiar_grilla">'+
			'<table>'+
			'<tr valign="bottom" height="45px">'+
			'<td>Menu desde(*)</td>'+
			'<td><input type="hidden" id="c_menu_desde" /><input type="text" id="d_menu_desde" class="text validate[required]" readonly="readonly" /></td>'+
			'<td><button type="button" id="bt_lupa_menu_desde" /></td>'+
			'</tr>'+
			'<tr valign="bottom" height="45px">'+
			'<td>N&deg; Grilla Desde(*)</td>'+
			'<td><input type="text" id="n_grilla_desde" class="text validate[required,custom[onlyIntNumber]]" readonly="readonly" /></td>'+
			'<td><button type="button" id="bt_lupa_n_grilla_desde" /></td>'+
			'</tr>'+
			'<tr valign="bottom" height="45px">'+
			'<td>Menu Hasta(*)</td>'+
			'<td><input type="hidden" id="c_menu_hasta" /><input type="text" id="d_menu_hasta" class="text validate[required]" readonly="readonly" /></td>'+
			'<td><button type="button" id="bt_lupa_menu_hasta" /></td>'+
			'</tr>'+
			'<tr valign="bottom" height="45px">'+
			'<td>N&deg; Grilla Hasta(*)</td>'+
			'<td><input type="text" id="n_grilla_hasta" class="text validate[required,custom[onlyIntNumber]]" /></td>'+
			'<td></td>'+
			'</tr>'+
			'<tr valign="bottom" height="45px">'+
			'<td colspan="2" align="right"><button type="button" id="bt_copiar_grilla" /></td>'+
			'<td></td>'+
			'</tr>'+
			'</table>'+
			'</form>'+
			...
			'<table>'+
			'<tr>'+
			'<td><textarea id="d_query_grid" rows="30" cols="140" readonly style="" ></textarea></td>'+
			'</tr>'+
			'</table>'+
			'</form>'+
			'</div>';

		var copiar_campos_tabla =
			'<div id="dialog_tomar_campos_tabla" title="Tomar los Campos de la Tabla" style="display:none;">'+
			"<table id='copiar_campos_desde_tabla_grid' class='scroll' cellpadding='0' cellspacing='0'><tr><td>&nbsp;</td></tr></table>"+
		...
		$('#main').append(content);
		$('#main').append(copiar_campos_tabla);
		...
		$("#detail_grid").jqGrid({
			...
			editurl: "../Funciones/maestro_abm_grid.php",
			sortname:'n_column',
			sortorder:'asc',
			rownumbers: true,
			height:400
...
//    alert(JSON.stringify(list));
	...
	function guardarOrdenCampos(){
		...
		if (id) {
			...
			$('#detail_grid tr.ui-widget-content.jqgrow').each(function(){//recorre todos los registros
