...
class DB_llamada_prc {
    ...
    function DB_llamada_prc($_id_menu,$_n_orden,$_tipo) {
    ...
    function load_llamada(){
        ...
        $db_query = new DB_Query("select d_validacion,d_procedure,id_menu_procedure,m_ora
        ...
        foreach($rows_columns as $row_column) {
            if($row_column['D_DATO'] != null){
                $llamada_prc .= str_replace(REEMP_QUERY,":".$row_column['D_PARAMETRO'],$row_column['D_DATO']).",";
            }else{
                $llamada_prc .= ":".$row_column['D_PARAMETRO'].",";
            }
        }
