...
$query="SELECT s.* FROM 
						(SELECT distinct me.id_menu id_menu,
												...
												m_framework, d_icono
									FROM menu me, usuarios_perfiles up, menu_perfiles mp, usuarios us, perfiles p
									WHERE me.c_tipo_menu != 'PAGINA'
									AND me.id_menu = mp.id_menu
									AND mp.id_perfil = up.id_perfil
									AND us.c_usuario = up.c_usuario
									AND upper(us.c_usuario) = upper(:p_usuario)
									AND me.m_visible = 'S'
									AND (me.c_letra_menu != 'J' or me.c_letra_menu is null)
...
foreach($row as $r){
    ...
    if ($r['C_TIPO_MENU']=="ITEM") {
        ...
        $menu.= 'class="nav-link" data-url="'.$v_entorno.'Aplicaciones/'.$r['D_URL'].'" data-param="'.str_replace('"','&quot;',json_encode($param_menu)).'"';
