...
*/
/**
* Modificacion en la agrupacion de datos
* Motivacion:		La agrupacion de datos se realiza con los datos de la pagina observada.
* Modificacion: 	La agrupacion de datos se realiza con una cantidad de datos definida como el minimo entre todos los registros existentes y una constante llamada TDI_MAX_ROWS_SIZE.
* Comentario:   	Se aplica cada vez que la grilla pasa de estado desagrupada a agrupada por alguno de sus campos.
* Implementacion:	Se hace uso de un array llamado tdi_grouped_grids que contiene el id de aquellas grillas agrupadas. 
*					Cuando la grilla se agrupa por uno de sus campos se añade su id al array y se determina la cantidad de registros por pagina en base a min(cant_total_regs, TDI_MAX_ROWS_SIZE).
*					En cambio cuando la grilla ya no cuenta con ningun campo agrupado su id se quita del array.
* Fecha: 			2018-03-02 
* Desarrollador: 	vmasrian@tdi.com.ar
*
*/
var TDI_MAX_ROWS_SIZE = 65536;
var tdi_grouped_grids = [];
var tdi_grouped_alert_type = "I";
var tdi_grouped_title = "Aviso";
var tdi_grouped_alert = "Se agrupara de a "+TDI_MAX_ROWS_SIZE+" registros.";
...
$.fn.jqGrid = function( pin ) {
	...
	return this.each( function() {
		...
		buildGrouping = function( index, isgroup ) {
			...
			if(isgroup !== -1) {				
			/**INICIO DESAGRUPAR TDI**/
					if (group.groupField.length == 1){
						var tdi_grid_id = '#'+$(ts).attr('id');
						var tdi_index_grid = tdi_grouped_grids.indexOf(tdi_grid_id);
						if (tdi_index_grid !== -1)
							tdi_grouped_grids.splice(tdi_grid_id, 1);
					}
			 /**FIN DESAGRUPAR TDI**/
		...
		buildColMenu = function( index, left, top ){
			...
			$("#column_menu > li > a").hover(
				...
				if(v === 'sortasc') {
					...
					/**INICIO TDI AGRUPACION**/ 
					var tdi_grid_id = '#'+$(ts).attr('id');
					if (tdi_grouped_grids.includes(tdi_grid_id) === false){
						var tdi_pager_id = $(tdi_grid_id).jqGrid('getGridParam', 'pager');
						var tdi_num_rows = $(tdi_grid_id).jqGrid('getGridParam', 'records');
						tdi_num_rows = (tdi_num_rows > TDI_MAX_ROWS_SIZE)?TDI_MAX_ROWS_SIZE:tdi_num_rows;
						$(tdi_grid_id).setGridParam({rowNum: tdi_num_rows});
						tdi_grouped_grids.push(tdi_grid_id);
						if ($(tdi_pager_id+' .ui-pg-selbox option[value="' +tdi_num_rows+ '"]').prop("selected", true).length == 0) {
							$(tdi_pager_id+" .ui-pg-selbox").append('<option role="option" value="'+tdi_num_rows+'" selected="selected">'+tdi_num_rows+'</option>');
							$(tdi_pager_id+" .ui-pg-selbox").val(tdi_num_rows);}
					}
					if(tdi_num_rows == TDI_MAX_ROWS_SIZE){
						mostrar_cuadro(tdi_grouped_alert_type, tdi_grouped_title,tdi_grouped_alert);}
			...
			buildRuleMenu = function( elem, left, top ){
				...
				$("#sopt_menu > li > a").hover(
					...
					if(p.autosearch===true){
						...
						$(inpelm).attr('readonly',false);
						$(inpelm).attr('disabled',false);
						$(inpelm).val('');
						if( $(inpelm).val() || v==="nu" || v ==="nn") {
							if(v==="nu"||v === "nn") {
								$(inpelm).attr('readonly',true);
								$(inpelm).attr('disabled',true);
							}
							triggerToolbar();
						}
					}
