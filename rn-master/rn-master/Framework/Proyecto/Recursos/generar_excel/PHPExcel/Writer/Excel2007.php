	...
	public function save($pFilename = null)
	{
		if ($this->_spreadSheet !== NULL) {
			...
			for ($i = 0; $i < $this->_spreadSheet->getSheetCount(); ++$i) {
				...
				//$this->getWriterPart('StringTable')->createStringTable($this->_spreadSheet->getSheet($i), $this->_stringTable);
			}
			...
			/*ob_start();
			echo("ContentType\n");
			echo($this->getWriterPart('ContentTypes')->writeContentTypes($this->_spreadSheet, $this->_includeCharts)."\n\n");
			ob_flush();
			echo("Relaciones\n");
			echo($this->getWriterPart('Rels')->writeRelationships($this->_spreadSheet)."\n\n");
			ob_flush();
			echo("WorkBook Relaciones\n");
			echo($this->getWriterPart('Rels')->writeWorkbookRelationships($this->_spreadSheet)."\n\n");
			ob_flush();
			echo("App\n");
			echo($this->getWriterPart('DocProps')->writeDocPropsApp($this->_spreadSheet)."\n\n");
			ob_flush();
			echo("Core\n");
			echo($this->getWriterPart('DocProps')->writeDocPropsCore($this->_spreadSheet)."\n\n");
			ob_flush();
			echo("Custom\n");
			echo($this->getWriterPart('DocProps')->writeDocPropsCustom($this->_spreadSheet)."\n\n");
			ob_flush();
			echo("Theme\n");
			echo($this->getWriterPart('Theme')->writeTheme($this->_spreadSheet)."\n\n");
			ob_flush();
			echo("SharedStrings\n");
			echo($this->getWriterPart('StringTable')->writeStringTable($this->_stringTable)."\n\n");
			ob_flush();
			echo("Styles\n");
			echo($this->getWriterPart('Style')->writeStyles($this->_spreadSheet)."\n\n");
			ob_flush();
			echo("WorkBook\n");
			echo($this->getWriterPart('Workbook')->writeWorkbook($this->_spreadSheet, $this->_preCalculateFormulas)."\n\n");
			ob_flush();
			echo("WorkSheet\n");
			echo($this->getWriterPart('Worksheet')->writeWorksheet($this->_spreadSheet->getSheet(0), $this->_stringTable, $this->_includeCharts)."\n\n");
			ob_flush();
			echo("Charts\n");
			$chartCount=0;
			if ($this->_includeCharts) {
				$charts = $this->_spreadSheet->getSheet(0)->getChartCollection();
				if (count($charts) > 0) {
					foreach($charts as $chart) {
						echo($this->getWriterPart('Chart')->writeChart($chart)."\n");
						$chartCount++;
					}
				}
			}
			ob_flush();
			echo("\nWorkSheet Relaciones\n");
			$chartRef1 = $chartRef2 = 0;
			for ($i = 0; $i < $this->_spreadSheet->getSheetCount(); ++$i) {

				// Add relationships
				echo("Relacion ".$i."\n");
				echo("\t".$this->getWriterPart('Rels')->writeWorksheetRelationships($this->_spreadSheet->getSheet($i), ($i + 1), $this->_includeCharts)."\n");
				...
				if (($drawingCount > 0) || ($chartCount > 0)) {
					...
					echo("\tDrawing relationships ".$i."\n");
					echo("\t\t".$this->getWriterPart('Rels')->writeDrawingRelationships($this->_spreadSheet->getSheet($i),$chartRef1, $this->_includeCharts)."\n");

					// Drawings
					echo("\tDrawings ".$i."\n");
					echo("\t\t".$this->getWriterPart('Drawing')->writeDrawings($this->_spreadSheet->getSheet($i),$chartRef2,$this->_includeCharts)."\n");
				}
				...
				if (count($this->_spreadSheet->getSheet($i)->getComments()) > 0) {
					// VML Comments
					echo("\tVML Comments ".$i."\n");
					echo("\t\t".$this->getWriterPart('Comments')->writeVMLComments($this->_spreadSheet->getSheet($i))."\n");

					// Comments
					echo("\tComments ".$i."\n");
					echo("\t\t".$this->getWriterPart('Comments')->writeComments($this->_spreadSheet->getSheet($i))."\n");
				}
				...
				if (count($this->_spreadSheet->getSheet($i)->getHeaderFooter()->getImages()) > 0) {
					// VML Drawings
					echo("\tVML Drawings ".$i."\n");
					echo("\t\t".$this->getWriterPart('Drawing')->writeVMLHeaderFooterImages($this->_spreadSheet->getSheet($i))."\n");

					// VML Drawing relationships
					echo("\tVML Drawing relationships ".$i."\n");
					echo("\t\t".$this->getWriterPart('Rels')->writeHeaderFooterDrawingRelationships($this->_spreadSheet->getSheet($i))."\n");

					// Media
					echo("\tMedia ".$i."\n");
					foreach ($this->_spreadSheet->getSheet($i)->getHeaderFooter()->getImages() as $image) {
						echo("\t\t".file_get_contents($image->getPath())."\n");
					}
				}
				ob_flush();
			}
