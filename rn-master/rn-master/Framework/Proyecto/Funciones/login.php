...
class oci_Logon {
    ...
    function oci_Logon() {
		...
		// Si tenemos la variable de sessioid seteamos la variable de contexto para la base
		if ($_SESSION['sessionid'] != ''){
			$query = "BEGIN 
                        PAC_VARIABLES_CTX.SETEAR ('SESSION_ID', :session_id);
                      END;";

			$stmt = oci_parse($this->DBCON,$query);
			ocibindbyname	($stmt,	":session_id",	$_SESSION['sessionid'] );
			oci_execute($stmt);
		}
